...
function resizeToken(element, width, height) {
  ...
  requestAnimationFrame(() => { // make sure it has rendered before measuring
    ...
    blockTokenElement.style.transform = `scaleX(${scale}) scaleY(${scaleY})`;
...
class Dropper {
  ...
  nextBlockUp() {
    ...
    while (this.state.tokenChain.length < this.tokenChainLength) {
      ...
      let blockElt = createBlockAt(block, this.tokenChainStart.left, this.tokenChainStart.top, this.cellWidth, this.cellHeight, this.colorBy);
      this.tokenElements.push(blockElt);
      this.state.tokenChain.unshift(blockElt);
    }
  ...
  nextTokenUp() {
    let tokenData = this.generateNextBlock();

    this.state.curY = 0;

    if (isDelete) {
      let rect = this.getColumnRect(this.state.curX, this.numRows - 1);
      this.state.currentBlock = createBlockAt(tokenData, rect.left, rect.top, rect.width, rect.height, this.colorBy);
      this.state.currentBlock.classList.add('delete');
    } else if (isBetweenToken) {
    ...
    this.tokenElements.push(this.state.currentBlock);
  }
