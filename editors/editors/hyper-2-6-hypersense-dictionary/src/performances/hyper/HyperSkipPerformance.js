...
export class HyperSkipPerformance extends SettingsMixin(class {}) {
    constructor(params={}) {
        ...
        this.settings = [
            ...
            { id: 'backgroundColor', name: 'Page Color', inBar: false, type: 'color', default: 'white', description: 'Color of the highlight on every Nth word' },
    ...
    initialize() {
        ...
        this._mousemoveHandler = (e) => {
            // If already hovered, check if still within any of its children
            if (this._lastHovered) {
                let stillInside = false;
                const children = this._lastHovered.querySelectorAll('.block');
                for (const child of children) {
                    const r = child.getBoundingClientRect();
                    if (e.clientX >= r.left && e.clientX <= r.right &&
                        e.clientY >= r.top && e.clientY <= r.bottom) {
                        stillInside = true;
                        if (this._lastActiveBlock !== child) {
                            if (this._lastActiveBlock) this._lastActiveBlock.classList.remove('active');
                            child.classList.add('active');
                            this._lastActiveBlock = child;
                        }
                        break;
                    }
                }
            ...
            const containers = this.overlay.querySelectorAll('.mask-container');
            let target = null;
            ...
            if (target !== this._lastHovered) {
                if (this._lastHovered) {
                    ...
                    this._unhoveredStream();
                }
                if (target) {
                    target.classList.add('hovered');
                    this._hoveredStream(target);
                }
        ...
        this._clickHandler = () => {
            if (this._lastActiveBlock && this._hoveredEntity) {
                const word = this._lastActiveBlock.querySelector('.block-word')?.textContent
                    || this._lastActiveBlock.textContent;
                this._selectWord(this._hoveredEntity, word);
            }
        };
    ...
    createOrUpdateStreams() {
        ...
        for (let index = 0; index < n; index++) {
            ...
            if (stream) {
                ...
                if (!stream._paused && reader.word !== tokenBox.text) {
                    if (!tokenBox.text) {
   ...
   _doPop(entity) {
    ...
    _findEntityByContainer(container) {
        for (const entity of this.state.streamEntitiesByKey.values()) {
            if (entity.component.container === container) return entity;
        }
        return null;
    }

    _hoveredStream(container) {
    ...
    _unhoveredStream() {
        const entity = this._hoveredEntity;
        if (!entity) return;
        this._hoveredEntity = null;
