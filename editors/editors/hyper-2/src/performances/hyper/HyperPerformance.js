...
export class HyperPerformance {
    ...
    callPop(entity, i) {
        ...
        setTimeout(() => {
            let synonymCount = entity.textStream.reader.getSynonymCount();
            ...
            if (shouldPop) {
                ...
                console.log('newestToken', newestToken, currentWord);
    ...
    drawOverlayWords() {
        ...
        this.state.wordBoxes.forEach((wordBox, i) => {
            ...
            // Check if stream tokens match current word or its synonyms
            // let shouldDraw = true;
            // if (stream && stream.textStream.tokens.length) {
            //     const validWords = new Set(stream.textStream.reader.synonyms);
            //     const allTokensValid = stream.textStream.tokens.every(t => validWords.has(t?.text));
            //     if (allTokensValid) {
            //         shouldDraw = false;
            //     }
            // }

            console.log('shouldDraw', {shouldDraw, wordBox, stream});
