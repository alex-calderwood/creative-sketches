export class ScoreTracker {
    constructor() {
        ...
        this.notes = [] // length limited, in order
    }
    ...
    /**
     * Get the last tracked note
     * @returns {number|null} - The last note or null if no notes
     */
    getLastNote() {
    ...
    /**
     * Compare a note with the last tracked note
     * @param {number} note - The note to compare
     * @returns {Object} - Object with isHigher and isLower properties
     */
    compareWithLast(note) {
        const lastNote = this.getLastNote();
        if (lastNote === null) {
            return { isHigher: false, isLower: false, isSame: false };
        }
        return {
            isHigher: note > lastNote,
            isLower: note < lastNote,
            isSame: note === lastNote
        };
    }
}
...
export class MidiInterface {
    ...
    static DEFAULT_SETTINGS = { noteRange: [0, 127] };

    constructor(play = true, options = {}) {
      ...
      this.settings = { ...MidiInterface.DEFAULT_SETTINGS, ...options };
    ...
    handleMidiMessage(message) {
      if (this.onMidiMessage) {
        const midiData = this.processMidiMessage(message);
        this.onMidiMessage(midiData);
      }
    }

    /**
     * Add score tracker information to MIDI data
     * @param {Object} midiData - Processed MIDI data
     */
    addScoreTrackerInfo(midiData) {
      // Add note comparison data
      const comparison = this.scoreTracker.compareWithLast(midiData.note);
      Object.assign(midiData, comparison);
      
      // Track the note if it's a note-on event
      if (midiData.eventType === 9 && midiData.velocity > 0) {
        this.scoreTracker.addNote(midiData.note);
      }
    }
    ...
    processMidiMessage(message) {
      ...
      this.addScoreTrackerInfo(midiData);

      if (this.verbosity.showNote) {
        console.log(`MIDI note: ${midiData.normalizedNote} - ${note} on channel ${channelNumber} with velocity ${velocity}  isHigher: ${midiData.isHigher} isLower: ${midiData.isLower} isSame: ${midiData.isSame}
          `);
